...
namespace UnityEditor.VisualScripting.Model.Translators
{
    class RootContext : TranslationContext
    {
        ...
        public void PrependStatement(StatementSyntax statementSyntax)
        {
        ...
        public ClassDeclarationSyntax Build(RoslynTranslator translator, VSGraphModel graphModel)
        {
            ...
            HashSet<ComponentQueryDeclarationModel> declaredQueries = new HashSet<ComponentQueryDeclarationModel>();
            foreach (var group in m_WrittenComponentsPerGroup)
            {
                declaredQueries.Add(group.Key.Query.ComponentQueryDeclarationModel);
                DeclareEntityQueries(group.Value);
            }

            // declare unused queries in case there's a Count Entities In Query node somewhere
            foreach (var graphVariable in graphModel.GraphVariableModels.OfType<ComponentQueryDeclarationModel>().Where(q => !declaredQueries.Contains(q)))
            {

...
namespace UnityEditor.VisualScripting.Editor
{
    public sealed class VseUIController
    {
        ...
        public void DisplayCompilationErrors(State state)
        {
            ...
            {
                var model = error.sourceNode;
                if (model == null || model.Destroyed)
                    continue;
                ...
                VseUtility.LogSticky(LogType.Error, LogOption.None, $"{graphAssetPath}: {error.description}", $"{graphAssetPath}@{error.sourceNodeGuid}", graphAsset.GetInstanceID());
            }
        }

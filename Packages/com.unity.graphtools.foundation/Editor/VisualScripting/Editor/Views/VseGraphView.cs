...
namespace UnityEditor.VisualScripting.Editor
{
    public class VseGraphView : GraphView, IDropTarget
    {
        ...
        internal enum CopyMode { DuplicateDuringCopy, KeepReference, }

        [Serializable]
        internal struct StackedNodesStruct
        {
            public AbstractNodeAsset parentStackNodeAsset;
            public AbstractNodeAsset stackedNodeModel;
        }
        ...
        {
            var copyPasteData = GatherCopiedElementsData(CopyMode.DuplicateDuringCopy, elements
        ...
        internal static void Duplicate(VSGraphModel graphModel, IReadOnlyCollection<IGraphElementModel> elementModels, out Dictionary<INodeModel, NodeModel> mapping)
        {
            ...
            CopyPasteData data = GatherCopiedElementsData(CopyMode.KeepReference, elementModels);
            if (data.IsEmpty())
            {
        ...
        internal static CopyPasteData GatherCopiedElementsData(CopyMode copyMode, IReadOnlyCollection<IGraphElementModel> graphElementModels)
        {
            bool clone = copyMode == CopyMode.DuplicateDuringCopy;
            ...
            List<ScriptableObject> nodesToCopy = originalNodes
                .Select(model => clone ? DuplicateElement(model.NodeAssetReference) : model.NodeAssetReference)
                .ToList();

            List<StackedNodesStruct> implicitStackedNodesToCopy = new List<StackedNodesStruct>();
            int nodeIndex = 0;
            foreach (var asset in nodesToCopy.OfType<AbstractNodeAsset>())
            {
                asset.Model.OriginalInstanceId = originalNodes.ElementAt(nodeIndex++).NodeAssetReference.GetInstanceID();
                if (asset is IStackAsset stackAsset)
                {
                    var stackModel = stackAsset.StackModel;
                    foreach (var stackedNodeModel in stackModel.NodeModels)
                    {
                        var stackedNodeAssetToCopy = (AbstractNodeAsset)(clone ? DuplicateElement(stackedNodeModel.NodeAssetReference) : stackedNodeModel.NodeAssetReference);
                        ...
                        stackedNodeAssetToCopy.Model.OriginalInstanceId = stackedNodeModel.NodeAssetReference.GetInstanceID();

                        implicitStackedNodesToCopy.Add(new StackedNodesStruct
                        {
                            parentStackNodeAsset = stackModel.NodeAssetReference,
            ...
            CopyPasteData copyPasteData = new CopyPasteData
            {
                ...
                stickyNotes = stickyNotesToCopy // TODO
            };

            return copyPasteData;

            ScriptableObject DuplicateElement(ScriptableObject o)
            {
                var cloned = Object.Instantiate(o);
                var copiedLength = cloned.name.Length - k_CloneStringLength;
                if (copiedLength > 0) cloned.name = cloned.name.Substring(0, copiedLength);
            ...
            {
                var outputNodeInstanceId = original.OutputPortModel.NodeModel.NodeAssetReference.GetInstanceID();
                var matchingNodes = nodesToCopy.Concat(implicitStackedNodesToCopy.Select(sns => sns.stackedNodeModel))
                    .OfType<AbstractNodeAsset>().Select(asset => asset.Model).ToList();
                ...
                var inputNodeInstanceId = original.InputPortModel.NodeModel.NodeAssetReference.GetInstanceID();
        ...
        {
            mapping = new Dictionary<INodeModel, NodeModel>();
            foreach (var original in copyPasteData.nodes.Cast<AbstractNodeAsset>())
            {
                if (!original)
                    continue;
                ...
                else // duplication OR not stacked
                {
                    PasteNode(targetInfo.OperationName, originalModel, mapping, originalModel.ParentStackModel as StackBaseModel,
                        ...
                        copyPasteData.implicitStackedNodes
                            .Where(x => x.parentStackNodeAsset == original).ToList());
                }
            }
        ...
        {
            var pastedNodeAsset = Object.Instantiate(copiedNode.NodeAssetReference);
            Undo.RegisterCreatedObjectUndo(pastedNodeAsset, "Duplicate node");

            var pastedNodeModel = (NodeModel)pastedNodeAsset.Model;
            pastedNodeModel.Title = copiedNode.Title;
            pastedNodeModel.Guid = GUID.Generate(); // give a different guid to copied node
                ...
                if (pastedNodeModel is StackBaseModel pastedStackModel)
                {
                    ...
                    if (implicitStackedNodes?.Count > 0)
                    {
                        foreach (var stackedNodeStruct in implicitStackedNodes)
                            PasteNode(operationName, stackedNodeStruct.stackedNodeModel.Model, mapping,
                    ...
                    if (copiedNode is FunctionModel copiedFunctionModel && pastedStackModel is FunctionModel pastedFunctionModel)
                    {
                        ...
                        {
                            VariableDeclarationModel parameterDeclaration = Object.Instantiate((VariableDeclarationModel)functionParameterModel);
            ...
            Undo.RegisterCreatedObjectUndo(pastedNodeAsset, operationName);

            if (pastedNodeParentStack != null)
            {
                Undo.RegisterCompleteObjectUndo(pastedNodeParentStack.NodeAssetReference, "Duplicate node");
                pastedNodeParentStack.AddStackedNode(pastedNodeModel, stackInsertionIndex);
            }

            if ((Object)graph.AssetModel)
                Utility.SaveAssetIntoObject(pastedNodeAsset, (Object)graph.AssetModel);
        }
...
//        }
        ...
        public override void AddToSelection(ISelectable selectable)
        {
            ...
            if (m_PersistedSelectionRestoreEnabled &&
                ...
                hasModel.GraphElementModel.SerializableAsset is Object obj)

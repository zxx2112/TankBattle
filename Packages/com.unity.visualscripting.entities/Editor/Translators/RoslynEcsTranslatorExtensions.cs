...
//                Debug.Log($"If in stack {statement.parentStackModel} Common descendant: {endStack}");
        ...
        [CanBeNull]
        public static ExpressionSyntax BuildArgumentList(this RoslynEcsTranslator translator, NodeModel call, out List<ArgumentSyntax> argumentList)
        {
            ExpressionSyntax instance = null;
            argumentList = new List<ArgumentSyntax>();
            foreach (IPortModel port in call.InputsById.Values)
            {
        ...
        public static IEnumerable<SyntaxNode> BuildGetInput(this RoslynEcsTranslator translator, GetInputNodeModel model, IPortModel portModel)
        {
            var methodName = model.MethodName(portModel);
            var call = GetInputTranslator.BuildCall(translator, model, methodName, out var inputName);

...
namespace Packages.VisualScripting.Editor.Elements
{
    public class BlackboardEcsProvider : IBlackboardProvider
    {
        ...
        {
            var componentQueriesSection = new BlackboardSection { title = k_ComponentQueriesSectionTitle };
            var queriesSectionHeader = componentQueriesSection.Q("sectionHeader");
            queriesSectionHeader.Add(new Button(OnAddComponentQueryButtonClicked) { name = "addButton", text = "+" });
        ...
        void OnAddComponentQueryButtonClicked()
        {
            if (m_Blackboard != null)
                AddItemRequested(m_Blackboard.Store, new CreateComponentQueryAction(k_DefaultComponentQueryName));
        }
        ...
        void OnAddVariableButtonClicked()
        {
            if (m_Blackboard != null)
                AddItemRequested(
                    m_Blackboard.Store,
                    new CreateGraphVariableDeclarationAction(k_DefaultVariableName, true, TypeHandle.Float));
        }
            ...
            foreach (BlackboardSection blackBoardSection in blackboard.Sections)
            {
                ...
                blackBoardSection.Query<BlackboardRow>().ForEach(row =>
                {
                    if (row.userData is IVariableDeclarationModel model && expandedRows.TryGetValue(model, out expandedValue))
                        row.expanded = expandedValue;
                    else if (row.userData is Tuple<IVariableDeclarationModel, bool> modelTuple && expandedRows.TryGetValue(modelTuple.Item1, out expandedValue))
                        row.expanded = expandedValue;
                });
